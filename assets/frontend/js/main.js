...
var Main = function() {
    ...
    var runFirstTwoWords = function() {
        $('.first-two-words').each(function() {
            var fullString = $(this).text().trim();
            var first2Words = fullString.replace(/(([^\s]+\s\s*){2})(.*)/, "$1");
            var replaceWord = "<span class='first-two-words-wrap' style='font-size: 0.5em;display: block;padding-bottom: 5px'>" + first2Words + "</span>";
            var newString = fullString.replace(first2Words, replaceWord);
            $(this).html(newString);
        });
    };

    var runFirstWords = function() {
        $('article .page-content h1').each(function() {
            var fullString = $(this).text().trim();
            var firstWords = fullString.replace(/(([^\s]+\s\s*){1})(.*)/, "$1");
            var replaceWord = "<span class='first-words-wrap' style='font-family: \"Heavenetica Cond 9\", Sans-Serif;color: #646161;font-weight: 600;'>" + firstWords + "</span>";
            var newString = fullString.replace(firstWords, replaceWord);
            $(this).html(newString);
        });
    };

    var runIsotope = function() {
        var myGrid = $('.isotope-wrap');
    ...
    var runColorbox = function() {
        ...
        $(".colorbox-group-1").colorbox({
            rel: 'image-popup-2',
            retinaImage: true,
            slideshow: true,
            width: "65%"
        });
        $(".colorbox-group-2").colorbox({
            rel: 'image-popup-3',
            retinaImage: true,
            slideshow: true,
            height: "90%"
